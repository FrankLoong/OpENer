...
extern void IApp_CloseSocket_udp(int pa_nSockFd);
...
/* private functions */
EIP_STATUS
ForwardOpen(S_CIP_Instance * pa_pstInstance,
            S_CIP_MR_Request * pa_MRRequest,
            S_CIP_MR_Response * pa_MRResponse);
EIP_STATUS
ForwardClose(S_CIP_Instance * pa_pstInstance,
             S_CIP_MR_Request * pa_MRRequest,
             S_CIP_MR_Response * pa_MRResponse);
...
assembleFWDOpenResponse(S_CIP_ConnectionObject * pa_pstConnObj,
                        S_CIP_MR_Response * pa_MRResponse,
                        EIP_UINT8 pa_nGeneralStatus,
                        EIP_UINT16 pa_nExtendedStatus);

EIP_STATUS
assembleFWDCloseResponse(EIP_UINT16 pa_ConnectionSerialNr,
                         EIP_UINT16 pa_OriginatorVendorID,
                         EIP_UINT32 pa_OriginatorSerialNr,
                         S_CIP_MR_Request * pa_MRRequest,
                         S_CIP_MR_Response * pa_MRResponse,
 ...
 */
EIP_STATUS
checkElectronicKeyData(EIP_UINT8 pa_nKeyFormat,
                       S_CIP_KeyData * pa_pstKeyData,
            ...
            S_CIP_MR_Response * pa_MRResponse)
{
  ...
  if(EIP_OK != tmp)
    {
      return assembleFWDOpenResponse(&g_stDummyConnectionObject, pa_MRResponse,
                                     (EIP_UINT8)tmp, nConnectionStatus);
    }
                    ...
                    EIP_UINT16 * pa_pnExtendedError)
{
  ...
  EIP_UINT8 nRemainingPathSize = pa_pstConnObj->ConnectionPathSize = *pnMsg++; /* length in words */
          ...
          while(nRemainingPathSize > 0)
            {
              ...
              switch(*pnMsg)
                ...
                case 0x80:
                  ...
                  nRemainingPathSize -= (EIP_UINT8)(g_unConfigDataLen + 2);
  ...
  if(0x03 != (pa_pstConnObj->TransportTypeClassTrigger & 0x03))
    {
      ...
      IApp_CloseSocket_udp(pa_pstConnObj->sockfd[CONSUMING]);
