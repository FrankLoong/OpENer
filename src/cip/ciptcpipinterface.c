...
configureNetworkInterface(const char * pa_acIpAdress,
                          const char * pa_acSubNetMask,
                          const char * pa_acGateway)
{
  EIP_UINT32 nHostId;
...
configureDomainName(const char * pa_acDomainName)
{
  ...
  Interface_Configuration.DomainName.Length = (EIP_INT16)strlen(pa_acDomainName);
...
configureHostName(const char * pa_acHostName)
{
  ...
  Hostname.Length = (EIP_UINT16)strlen(pa_acHostName);
  if(Hostname.Length)
    {
                              ...
                              S_CIP_MR_Request * pa_stMRRequest, S_CIP_MR_Response * pa_stMRResponse)
{
  ...
  pa_stMRResponse->DataLength = (EIP_INT16)(pa_stMRResponse->Data - ptmp);
  pa_stMRResponse->Data = ptmp;
