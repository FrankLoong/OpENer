...
handleDataOnTCPSocket(int pa_nSocket)
{
  ...
  nCheckVal = recv(pa_nSocket, (char*)g_acPCEthernetCommBuffer, 4, 0); /*TODO we may have to set the socket to a non blocking socket */
  ...
  if(PC_OPENER_ETHERNET_BUFFER_SIZE - 4 < unDataSize)
    {
      ...
      do
        {
          nCheckVal = recv(pa_nSocket, (char*)g_acPCEthernetCommBuffer, nDataSent, 0);
  ...
  if(true == checkSocketSet(TheNetworkStatus.nUDPListener))
    {
      ...
      nReceived_size = recvfrom(TheNetworkStatus.nUDPListener,
                                (char*)g_acPCEthernetCommBuffer, PC_OPENER_ETHERNET_BUFFER_SIZE, 0,
